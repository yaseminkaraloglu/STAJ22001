...
{
    [Route("apps/Seyahat/latest/api/DataSource/[action]")]
    [Route("apps/Seyahat/{v:int:min(1)}/api/DataSource/[action]")]
    ...
    public class DataSourceController : BaseDataSourceController 
    {
        ...
        [HttpPost]
[AcceptVerbs("POST")]
[ActionName("SehirleriGetir")]
public async Task<object> SehirleriGetir_Action([FromBody] SehirleriGetirRequest request)
{
    return await SehirleriGetir(request);
}
...
[HttpPost]
[AcceptVerbs("POST")]
[ActionName("UcusStandartiGetir")]
public async Task<object> UcusStandartiGetir_Action([FromBody] UcusStandartiGetirRequest request)
{
    return await UcusStandartiGetir(request);
}

        ///Handles
        internal async Task<DataSourceResponse<object>> SehirleriGetir(SehirleriGetirRequest request)
{
    string queryName = "SehirleriGetir";
...
internal async Task<DataSourceResponse<object>> UcusStandartiGetir(UcusStandartiGetirRequest request)
{
    string queryName = "UcusStandartiGetir";
    ...
    return response;
        ...
        ///Iterators
        [NonAction]
public IEnumerable<Dictionary<string, object>> SehirleriGetirIterator(SehirleriGetirRequest request)
{
    int pagingSkip = 0;
    int pagingTake = 100;
    int pageIndex = 0;
    ...
    while (!endOfSource)
    {
        ...
        var response = SehirleriGetir(request).Result;
...
[NonAction]
public IEnumerable<Dictionary<string, object>> UcusStandartiGetirIterator(UcusStandartiGetirRequest request)
{
    int pagingSkip = 0;
    int pagingTake = 100;
    int pageIndex = 0;
    ...
    while (!endOfSource)
    {
        ...
        var response = UcusStandartiGetir(request).Result;
        endOfSource = response.IsEndOfSource;
        if (response.Success && response.Result != null)
        {
            if (response.Result is List<Dictionary<string, object>> list)
